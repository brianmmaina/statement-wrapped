...
DESCRIPTION_COLUMNS = ("description", "merchant", "details")
AMOUNT_COLUMNS = ("amount", "transaction amount")


class AppleCardParser:
    def parse(self, file):
        ...
        if not date_col or not amount_col:
            raise ValueError(f"Apple Card CSV must have date and amount. Found: {list(first.keys())}")
        ...
        for row in rows:
            ...
            if date_val is None or amount_val is None:
                continue
            # Apple Card CSV: negative = spending/charges. Normalize to canonical (positive = outflow, negative = inflow).
