...
DESCRIPTION_COLUMNS = ("description", "payee", "details")
...
class BoAParser:
    def parse(self, file):
        ...
        if not date_col or not amount_col:
            raise ValueError(f"BoA CSV must have date and amount. Found: {list(first.keys())}")
        ...
        for row in rows:
            ...
            if date_val is None or amount_val is None:
                continue
            # BoA CSV: negative = spending, positive = income. Normalize to canonical (positive = outflow, negative = inflow).
